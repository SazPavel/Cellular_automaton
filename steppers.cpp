...
int main(){
	...
	for(int i = N/2 + n; i < N/2 + 5*n; i++){
		tile[i].color = level[i] = rand()%2;
		if(tile[i].color > 0)
			tile[i].is = 1;
	}
    ...
    while (window.isOpen()){
        ...
        for(int i = 0; i < N; i++){
        	if(tile[i].left != NULL ){
        		next += tile[i].left->is;
        		if(tile[i].left->color == 1)
        			t = 1;
			}
        	if(tile[i].right != NULL){
				next +=	tile[i].right->is;
        		if(tile[i].right->color == 1)
        			t = 1;
			}
        	if(tile[i].up != NULL ){
        		next += tile[i].up->is;
        		if(tile[i].up->color == 1)
        			t = 1;
			}
        	if(tile[i].down != NULL){
				next +=	tile[i].down->is;
        		if(tile[i].down->color == 1)
        			t = 1;
			}
				
        	if(tile[i].upleft != NULL ){
        		next += tile[i].upleft->is;
        		if(tile[i].upleft->color == 1)
        			t = 1;
			}
        	if(tile[i].upright != NULL){
				next +=	tile[i].upright->is;
        		if(tile[i].upright->color == 1)
        			t = 1;
			}
        	if(tile[i].downleft != NULL ){
        		next += tile[i].downleft->is;
        		if(tile[i].downleft->color == 1)
        			t = 1;
			}
        	if(tile[i].downright != NULL){
				next +=	tile[i].downright->is;
        		if(tile[i].downright->color == 1)
        			t = 1;
			}	
			tile[i].next_color = tile[i].color;	
			tile[i].next_is = tile[i].is;	
	   		if(next == 3 && tile[i].color == 0){
        		tile[i].next_color = 1;	
        		tile[i].next_is = 1;
			}
	   		if(next >= 4 && tile[i].color == 2){
        		tile[i].next_color = 0;	
        		tile[i].next_is = 0;
			}
	   		if(next >= 3 && tile[i].color == 2 && t == 1){
        		tile[i].next_color = 0;	
        		tile[i].next_is = 0;
			}
	   		if(next < 2 && tile[i].color == 2){
        		tile[i].next_color = 0;	
        		tile[i].next_is = 0;
			}
        	if(tile[i].color == 1){
