    ...
    while (window.isOpen()){
        ...
        for(int i = 0; i < N; i++){
        	...
        	if(tile[i].downright != NULL)
	   		...
	   		if(next == tile[i].color * 3 || tile[i].color == 1){
        		tile[i].next_color = tile[i].color + 1;
				if(tile[i].next_color > 3)
					tile[i].next_color = 3;
        		
			   }
        	if(next > tile[i].color * 3 || next < tile[i].color * 2){
				tile[i].next_color = tile[i].color - 1;	
				if(tile[i].next_color < 0)
					tile[i].next_color = 0;
			}
        	next = 0;			
		}  
